...
__all__ = [
    "convolution_mapping_blockvisibility",
...
def convolution_mapping_blockvisibility(
...
def spatial_mapping(griddata, u, v, w, cf=None):
    ...
    if cf is not None:
        ...
        assert (
            numpy.min(pu_grid) >= 0
        ), "image sampling wrong: U axis underflows: %f" % numpy.min(pu_grid)
        assert (
            numpy.max(pu_grid) < griddata.griddata_acc.shape[2]
        ), "U axis overflows: %f" % numpy.max(pu_grid)
        assert (
            numpy.min(pv_grid) >= 0
        ), "image sampling wrong: V axis underflows: %f" % numpy.min(pv_grid)
        assert (
            numpy.max(pv_grid) < griddata.griddata_acc.shape[3]
        ), "V axis overflows: %f" % numpy.max(pv_grid)
        ...
        assert (
            numpy.min(pu_grid) >= 0
        ), "Cellsize is too large: U axis underflows: %f" % numpy.min(pu_grid)
        assert (
            numpy.max(pu_grid) < griddata.griddata_acc.shape[2]
        ), "U axis overflows: %f" % numpy.max(pu_grid)
        assert (
            numpy.min(pv_grid) >= 0
        ), "Cellsize is too large: V axis underflows: %f" % numpy.min(pv_grid)
        assert (
            numpy.max(pv_grid) < griddata.griddata_acc.shape[3]
        ), "V axis overflows: %f" % numpy.max(pv_grid)
...
def grid_blockvisibility_to_griddata(vis, griddata, cf):
    ...
    fvist = numpy.nan_to_num(
        vis.blockvisibility_acc.flagged_vis.reshape(
            [nrows * nbaselines, nvchan, nvpol]
        ).T
    )
    fwtt = numpy.nan_to_num(
        vis.blockvisibility_acc.flagged_imaging_weight.reshape(
            [nrows * nbaselines, nvchan, nvpol]
        ).T
    )
...
def grid_blockvisibility_weight_to_griddata(vis, griddata: GridData):
    ...
    for vchan in range(nvchan):
        imchan = vis_to_im[vchan]
        pu_grid, pv_grid = convolution_mapping_blockvisibility(vis, griddata, vchan)
        for pol in range(nvpol):
            for row in range(nrows * nbaselines):
...
def griddata_blockvisibility_reweight(
    ...
    for vchan in range(nvchan):
        ...
        for pol in range(nvpol):
            for row in range(nrows * nbaselines):
                ...
                fvist[pol, vchan, row] = numpy.einsum(
                    "ij,ij", subgrid, subcf
                )  # / numpy.sum(subcf.real)
