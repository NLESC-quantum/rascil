...
def reduce_sum(img_list):
    if len(img_list) == 1:
        return rsexecute.execute(lambda x: x)(img_list[0])
    elif len(img_list) == 2:
        return rsexecute.execute(add)(img_list[0], img_list[1])
    else:
        centre = len(img_list) // 2
        return reduce_sum(
            [reduce_sum(img_list[:centre]), reduce_sum(img_list[centre:])]
        )
...
def source_region_mask(img, use_dask="False"):
    ...
    if use_dask == "True":
        ...
        source_regions_sum = reduce_sum(source_regions_list)
