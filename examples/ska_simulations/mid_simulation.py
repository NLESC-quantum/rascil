...
def make_images_workflow(args, bvis_list, state):
    ...
    npixel = args.npixel
    advice = [
        rsexecute.execute(advise_wide_field)(bvis, verbose=False) for bvis in bvis_list
    ]
    ...
    dirty_list = invert_list_rsexecute_workflow(
        bvis_list, template_model_imagelist=model_list, context=imaging_context
    )
    ...
    dirtyname = "{results}/SKA_{configuration}_SIM_{duration}_{band}_dec_{dec:.1f}_{mode}_{state}_dirty.fits".format(
...
def simulation(args):
    ...
    logfile = "{results}/SKA_{configuration}_SIM_{duration}_{band}_dec_{dec:.1f}_{mode}.log".format(
        configuration=args.configuration,
        results=args.results,
        band=band,
        dec=declination,
    ...
    entire_bvis = create_blockvisibility(
        mid,
        times,
        frequency=frequency,
    ...
    time_slices = len(
        numpy.arange(time_range[0] * 3600, time_range[1] * 3600, time_chunk)
    )
    ...
    comps_name = (
        "{results}/SKA_{configuration}_SIM_{band}_dec_{dec:.1f}_components.hdf5".format(
            configuration=args.configuration,
            results=args.results,
            band=band,
            dec=declination,
    ...
    vp_list = [
        rsexecute.execute(create_vp)(
            vp, pbtype, pointingcentre=phasecentre, use_local=not use_radec
        )
        for vp in vp_list
    ]
    ...
    if mode == "random_pointing":
        ...
        no_error_gtl, error_gtl = create_pointing_errors_gaintable_rsexecute_workflow(
            bvis_list,
            original_components,
        ...
        no_error_gtl, error_gtl = create_surface_errors_gaintable_rsexecute_workflow(
            band,
            bvis_list,
            original_components,
        ...
        no_error_gtl, error_gtl = create_polarisation_gaintable_rsexecute_workflow(
            band, bvis_list, original_components, basename=basename, show=False
        )
    else:
    ...
    def concat_write(bvis, state):
        msname = "{results}/SKA_{configuration}_SIM_{duration}_{band}_dec_{dec:.1f}_{mode}_{state}.ms".format(
            configuration=args.configuration,
            results=args.results,
            band=band,
            dec=declination,
        ...
        log.info(
            "Writing {mode} {state} visibility to {msname}".format(
                mode=mode, state=state, msname=msname
            )
        )
    ...
    error_bvis = rsexecute.execute(visibility_gather_time)(
        error_bvis_list, error_bvis, vis_slices=time_slices
    )
    ...
    no_error_bvis = rsexecute.execute(visibility_gather_time)(
        no_error_bvis_list, no_error_bvis, vis_slices=time_slices
    )
    ...
    difference_bvis_list = subtract_list_rsexecute_workflow(
        error_bvis_list, no_error_bvis_list
    )
    difference_bvis_list = make_images_workflow(
        args, difference_bvis_list, "difference"
    )
    ...
    rsexecute.save_statistics(
        "{results}/SKA_{configuration}_SIM_{duration}_{band}_dec_{dec:.1f}_{mode}".format(
            configuration=args.configuration,
            results=args.results,
            band=band,
            dec=declination,
...
def cli_parser():
    ...
    parser.add_argument(
        "--configuration", type=str, default="MID", help="Configuration: MID | MEERKAT+"
    )
    parser.add_argument(
        "--nchan", type=int, default=1, help="Number of frequency channels"
    )
    parser.add_argument(
        "--channel_width", type=float, default=None, help="Channel bandwidth (Hz)"
    )
    parser.add_argument(
        "--integration_time", type=float, default=180, help="Integration time (s)"
    )
    parser.add_argument(
        "--time_range",
        type=float,
    ...
    parser.add_argument(
        "--imaging_context", type=str, default=None, help="Imaging context '2d' | 'ng'"
    )
    ...
    parser.add_argument(
        "--pbtype", type=str, default="MID", help="Primary beam model: MID or MID_GAUSS"
    )
    ...
    # Control parameters
    parser.add_argument(
        "--use_radec", type=str, default="False", help="Calculate in RADEC (false)?"
    )
    parser.add_argument(
        "--shared_directory",
        type=str,
    ...
    parser.add_argument(
        "--dynamic_pe", type=float, default=1.0, help="Multiplier for dynamic errors"
    )
    parser.add_argument(
        "--pointing_directory",
        type=str,
