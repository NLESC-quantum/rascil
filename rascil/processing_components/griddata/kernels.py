...
def create_box_convolutionfunction(im):
    ...
    gcf_image = create_image_from_array(
        gcf_data, cf.projection_wcs, im.polarisation_frame
    )
...
def create_pswf_convolutionfunction(im, oversampling=127, support=8):
    ...
    for y in range(oversampling):
        for x in range(oversampling):
            cf.data[:, :, 0, y, x, :, :] = numpy.outer(kernel[y, :], kernel[x, :])[
                numpy.newaxis, numpy.newaxis, ...
            ]

    for y in range(oversampling):
        for x in range(oversampling):
            norm = numpy.sum(numpy.real(cf.data[:, :, 0, y, x, :, :]), axis=(-2, -1))[
                ..., numpy.newaxis, numpy.newaxis
            ]
            cf.data[:, :, 0, y, x, :, :] /= norm
...
def create_awterm_convolutionfunction(
    ...
    if use_aaf:
        this_pswf_gcf, _ = create_pswf_convolutionfunction(
            subim, oversampling=1, support=6
        )
    ...
    for z, w in enumerate(w_list):
        ...
        for y in range(oversampling):
            ...
            for x in range(oversampling):
                ...
                cf.data[..., z, y, x, :, :] = paddedplane.data[
    ...
    if normalise:
        ...
        for y in range(oversampling):
            for x in range(oversampling):
                # uu = range(xbeg, xend, -oversampling)
                norm[..., y, x] = numpy.sum(
                    numpy.real(cf.data[:, :, 0, y, x, :, :]), axis=(-2, -1)
                )
        for z, _ in enumerate(w_list):
            for y in range(oversampling):
                for x in range(oversampling):
                    cf.data[:, :, z, y, x] /= norm[..., y, x][
                        ..., numpy.newaxis, numpy.newaxis
                    ]
...
def create_vpterm_convolutionfunction(
    ...
    for y in range(oversampling):
        ...
        for x in range(oversampling):
            ...
            # uu = range(xbeg, xend, -oversampling)
            cf.data[..., 0, y, x, :, :] = paddedplane.data[
                ..., ybeg:yend:-oversampling, xbeg:xend:-oversampling
            ]

    if normalise:
        cf.data /= numpy.sum(
            numpy.real(cf.data[0, 0, 0, oversampling // 2, oversampling // 2, :, :])
        )
    cf.data = numpy.conjugate(cf.data)
...
def convert_image_to_kernel(im: Image, oversampling, kernelwidth):
    ...
    assert im.wcs.wcs.ctype[0] == "UU", (
        "Axis type %s inappropriate for construction of kernel" % im.wcs.wcs.ctype[0]
    )
    assert im.wcs.wcs.ctype[1] == "VV", (
        "Axis type %s inappropriate for construction of kernel" % im.wcs.wcs.ctype[1]
    )
