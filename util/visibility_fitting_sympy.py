...
fold_conjugates = Transform(
    lambda f: 2 * re(f.args[0]),
    lambda f: isinstance(f, Add)
    and len(f.args) == 2
    and f.args[1] == f.args[0].conjugate(),
)

fold_conjugates_2 = Transform(
    lambda f: f.args[0] + 2 * re(f.args[1]),
    lambda f: isinstance(f, Add)
    and len(f.args) == 3
    and f.args[2] == f.args[1].conjugate(),
)
