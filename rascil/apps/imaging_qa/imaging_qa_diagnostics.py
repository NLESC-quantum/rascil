...
def quality_image_bdsf(im_data, description="image"):
...
def scatter_mask_list(gaussian_list, grid, beam_radius, image_shape):
    """
    ...
    :return source_regions: souce regions
    """
...
def gather_image_list(img_list):
    ...
    :return gather_img_list: image list
    """
    if len(img_list) == 1:
        return rsexecute.execute(lambda x: x)(img_list[0])
    elif len(img_list) == 2:
        return rsexecute.execute(lambda a, b: a | b)(img_list[0], img_list[1])
    else:
        centre = len(img_list) // 2
        return gather_image_list(
            [gather_image_list(img_list[:centre]), gather_image_list(img_list[centre:])]
        )


# TODO: This function is costly and needs to be revisited
def source_region_mask(img, use_dask=False):
    """
    Mask pixels from an image which are within 5*beam_width of sources in the
    source catalogue.

    :param img: PyBDSF image object to be masked
    :param use_dask: Boolean, if using DASK
...
def imaging_qa_diagnostics(bdsf_image, input_image, image_type, use_dask=False):
    """
    ...
    if image_type == "residual":
        ...
        residual_stats = quality_image_bdsf(
        ...
        source_mask, background_mask = source_region_mask(bdsf_image, use_dask=use_dask)

        sources_stats = quality_image_bdsf(source_mask, description="sources")
        background_stats = quality_image_bdsf(background_mask, description="background")
        restored_stats = quality_image_bdsf(
