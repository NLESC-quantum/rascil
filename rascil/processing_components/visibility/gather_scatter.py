...
def visibility_gather(
    visibility_list: List[Visibility], vis: Visibility, vis_iter, vis_slices=None
) -> Visibility:
    ...
    for i, rows in enumerate(rowses):
        ...
        if visibility_list[i] is not None and sum_rows > 0:
            assert (
                sum_rows == visibility_list[i].nvis
            ), "Mismatch in number of rows (%d, %d) in gather for slice %d" % (
                int(sum_rows),
                visibility_list[i].nvis,
                i,
            )
...
def visibility_gather_w(
    visibility_list: List[Visibility], vis: Visibility, vis_slices=1
) -> Visibility:
    ...
    """
    assert isinstance(vis, Visibility), vis
    return visibility_gather(
        visibility_list, vis, vis_iter=vis_wslice_iter, vis_slices=vis_slices
    )


def visibility_gather_time(
    visibility_list: List[Visibility], vis: Visibility, vis_slices=1
) -> Visibility:
    ...
    """
    return visibility_gather(
        visibility_list, vis, vis_iter=vis_timeslice_iter, vis_slices=vis_slices
    )
...
def visibility_gather_channel(
    vis_list: List[BlockVisibility], vis: BlockVisibility = None
):
    ...
    if vis is None:
        ...
        gathered_channel_bandwidth = numpy.concatenate(
            [v.channel_bandwidth for v in vis_list]
        )
