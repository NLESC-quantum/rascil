...
def image_is_canonical(im: Image):
    ...
    canonical = (
        canonical
        and im.wcs.wcs.ctype[0] == "RA---SIN"
        and im.wcs.wcs.ctype[1] == "DEC--SIN"
    )
    canonical = canonical and im.wcs.wcs.ctype[2] == "STOKES"
    canonical = canonical and (
        im.wcs.wcs.ctype[3] == "FREQ" or im.wcs.wcs.ctype[3] == "MOMENT"
    )
...
def export_image_to_fits(im: Image, fitsfile: str = "imaging.fits"):
    ...
    if im.data.dtype == "complex":
        return fits.writeto(
            filename=fitsfile,
            data=numpy.real(im.data),
            header=im.wcs.to_header(),
            overwrite=True,
        )
    else:
        return fits.writeto(
            filename=fitsfile, data=im.data, header=im.wcs.to_header(), overwrite=True
        )
...
def import_image_from_fits(fitsfile: str, fixpol=True) -> Image:
    ...
    if len(fim.data) == 2:
        ...
        try:
            ...
            if fixpol:
                permute = fim.polarisation_frame.fits_to_rascil[
                    fim.polarisation_frame.type
                ]
...
def reproject_image(im: Image, newwcs: WCS, shape=None) -> (Image, Image):
    ...
    if len(im.shape) == 4:
            ...
            for chan in range(nchan):
                for pol in range(npol):
                    rep_real[chan, pol], foot[chan, pol] = reproject_interp(
                        (im.data.real[chan, pol], im.wcs.sub(2)),
                        newwcs.sub(2),
                        shape[2:],
                        order="bicubic",
                    )
                    rep_imag[chan, pol], foot[chan, pol] = reproject_interp(
                        (im.data.imag[chan, pol], im.wcs.sub(2)),
                        newwcs.sub(2),
                        shape[2:],
                        order="bicubic",
                    )
        ...
        if im.data.dtype == "complex":
            rep_real, foot = reproject_interp(
                (im.data.real, im.wcs), newwcs, shape, order="bicubic"
            )
            rep_imag, foot = reproject_interp(
                (im.data.imag, im.wcs), newwcs, shape, order="bicubic"
            )
            rep = rep_real + 1j * rep_imag
        else:
            rep, foot = reproject_interp(
                (im.data, im.wcs), newwcs, shape, order="bicubic"
            )
    ...
    return (
        create_image_from_array(rep, newwcs, im.polarisation_frame),
        create_image_from_array(foot, newwcs, im.polarisation_frame),
    )
...
def show_components(im, comps, npixels=128, fig=None, vmax=None, vmin=None, title=""):
    ...
    for isc, sc in enumerate(comps):
        ...
        centre = numpy.round(
            skycoord_to_pixel(sc.direction, newim.wcs, 1, "wcs")
        ).astype("int")
        ...
        plt.imshow(
            newim.data[0, 0, ...], origin="lower", cmap="Greys", vmax=vmax, vmin=vmin
        )
...
def smooth_image(model: Image, width=1.0, normalise=True):
    ...
    for pol in range(npol):
        for chan in range(nchan):
            cmodel.data[chan, pol, :, :] = convolve_fft(
                model.data[chan, pol, :, :],
                kernel,
...
def calculate_image_from_frequency_moments(
    ...
    assert (
        moment_image.wcs.wcs.ctype[3] == "MOMENT"
    ), "Second image should be a moment image"
...
def convert_polimage_to_stokes(im: Image, complex_image=False, **kwargs):
    ...
    if im.polarisation_frame == PolarisationFrame("linear"):
        ...
        return create_image_from_array(
            to_required(cimarr), im.wcs, PolarisationFrame("stokesIQUV")
        )
    elif im.polarisation_frame == PolarisationFrame("linearnp"):
        cimarr = convert_linear_to_stokes(im.data)
        return create_image_from_array(
            to_required(cimarr), im.wcs, PolarisationFrame("stokesIQ")
        )
        ...
        cimarr = convert_circular_to_stokes(im.data)
        return create_image_from_array(
            to_required(cimarr), im.wcs, PolarisationFrame("stokesIV")
        )
...
def create_image(
    ...
    return create_image_from_array(
        numpy.zeros(shape, dtype=dtype), w, polarisation_frame=polarisation_frame
    )


def create_image_from_array(
    data: numpy.array, wcs: WCS, polarisation_frame: PolarisationFrame
) -> Image:
...
def create_empty_image_like(im: Image) -> Image:
    ...
    if image_sizeof(im) >= 1.0:
        log.debug(
            "create_empty_image_like: created %s image of shape %s, size %.3f (GB)"
            % (fim.data.dtype, str(fim.shape), image_sizeof(fim))
        )
    assert isinstance(fim, Image), "Type is %s" % type(fim)
    return fim
...
def fft_image(im, template_image=None):
    ...
    if im.wcs.wcs.ctype[0] == "RA---SIN" and im.wcs.wcs.ctype[1] == "DEC--SIN":
        ...
        im.wcs.wcs.ctype[0] == "AZELGEO long" and im.wcs.wcs.ctype[1] == "AZELGEO lati"
    ):
        ft_wcs.wcs.axis_types[0] = 0
        ft_wcs.wcs.axis_types[1] = 0
        ft_wcs.wcs.crval[0] = 0.0
        ...
        raise NotImplementedError(
            "Cannot FFT specified axes {0}, {1}".format(
                im.wcs.wcs.ctype[0], im.wcs.wcs.ctype[1]
            )
        )
    ...
    if im.shape == shape:
        ...
        for axis, _ in enumerate(im.shape):
            if shape[axis] < im.shape[axis]:
                raise ValueError(
                    "Padded shape %s is smaller than input shape %s" % (shape, im.shape)
                )
...
def create_w_term_like(
    ...
    fim = create_image_from_array(
        fim_array, wcs=im.wcs, polarisation_frame=im.polarisation_frame
    )
...
def scale_and_rotate_image(im, angle=0.0, scale=None, order=5):
    ...
    for chan in range(nchan):
        for pol in range(npol):
            if im.data.dtype == "complex":
                newim.data[chan, pol] = affine_transform(
                    im.data[chan, pol].real,
                    inv_transform,
                    offset=offset,
                    order=order,
                    output_shape=(ny, nx),
                ) + 1.0j * affine_transform(
                    im.data[chan, pol].imag,
                    inv_transform,
                    offset=offset,
                    order=order,
                    output_shape=(ny, nx),
...
def rotate_image(im, angle=0.0, order=5):
    ...
    if newim.data.dtype == "complex":
        newim.data = rotate(
            im.data.real, angle=numpy.rad2deg(angle), axes=(-2, -1), order=order
        ) + 1j * rotate(
            im.data.imag, angle=numpy.rad2deg(angle), axes=(-2, -1), order=order
        )
    else:
        newim.data = rotate(
            im.data, angle=numpy.rad2deg(angle), axes=(-2, -1), order=order
        )
    return newim
...
def apply_voltage_pattern_to_image(
    ...
    if is_scalar:
        ...
        if inverse:
            for chan in range(nchan):
                pb = (
                    vp.data[chan, 0, ...] * numpy.conjugate(vp.data[chan, 0, ...])
                ).real
