...
def get_frequency_map(vis, im: Image = None):
    ...
    if im is None:
        ...
        assert (
            min(vfrequencymap) >= 0
        ), "Invalid frequency map: visibility channel < 0: %s" % str(vfrequencymap)
        ...
        v2im_map = (
            im.image_acc.wcs.sub(["spectral"])
            .wcs_world2pix(ufrequency, 0)[0]
            .astype("int")
        )
        ...
        assert (
            min(vfrequencymap) >= 0
        ), "Invalid frequency map: image channel < 0 %s" % str(vfrequencymap)
        assert (
            max(vfrequencymap) < im["pixels"].data.shape[0]
        ), "Invalid frequency map: image channel > number image channels %s" % str(
            vfrequencymap
        )
...
def get_polarisation_map(vis: BlockVisibility, im: Image = None):
    ...
    if vis.blockvisibility_acc.polarisation_frame == im.image_acc.polarisation_frame:
        ...
        elif vis.blockvisibility_acc.polarisation_frame == PolarisationFrame(
            "stokesIQUV"
        ):
