...
def predict_list_rsexecute_workflow(
    ...
    if get_parameter(kwargs, "use_serial_predict", False):
        ...
        return [
            rsexecute.execute(predict_list_serial_workflow, nout=1)(
                vis_list=[vis_list[i]],
                model_imagelist=[model_imagelist[i]],
    ...
    if gcfcf is None:
        gcfcf = [
            rsexecute.execute(create_pswf_convolutionfunction)(m)
    ...
    if facets == 1:
        ...
        for ivis, subvis in enumerate(vis_list):
            ...
            for sub_vis_list in sub_vis_lists:
                ...
                image_vis_list = rsexecute.execute(
                    predict_ignore_none, pure=True, nout=1
            ...
            image_results_list.append(
                rsexecute.execute(visibility_gather, nout=1)(
        ...
        for ivis, subvis in enumerate(vis_list):
            ...
            for sub_vis_list in sub_vis_lists:
                ...
                for facet_list in facet_lists:
                    ...
                    facet_vis_list = rsexecute.execute(
                        predict_ignore_none, pure=True, nout=1
                ...
                facet_vis_lists.append(
                    rsexecute.execute(sum_predict_results)(facet_vis_results)
                )
            # Sum all sub-visibilities
            image_results_list_list.append(
                rsexecute.execute(visibility_gather, nout=1)(
                    facet_vis_lists, subvis, vis_iter
                )
            )
...
def invert_list_rsexecute_workflow(
    ...
    **kwargs
    ...
    if get_parameter(kwargs, "use_serial_invert", False):
        ...
        return [
            rsexecute.execute(invert_list_serial_workflow, nout=1)(
                vis_list=[vis_list[i]],
                template_model_imagelist=[template_model_imagelist[i]],
                context=context,
                dopsf=dopsf,
                normalize=normalize,
                vis_slices=vis_slices,
    ...
    def gather_image_iteration_results(results, template_model):
        ...
        for dpatch in image_scatter_facets(
            result, facets=facets, overlap=overlap, taper=taper
        ):
        ...
        flat = image_gather_facets(
            results, flat, facets=facets, overlap=overlap, taper=taper, return_flat=True
        )
    ...
    if gcfcf is None and facets == 1:
        ...
        gcfcf = [
            rsexecute.execute(create_pswf_convolutionfunction)(
    ...
    if facets == 1:
        for ivis, sub_vis_list in enumerate(vis_list):
            ...
            for sub_sub_vis_list in sub_sub_vis_lists:
                vis_results.append(
                    rsexecute.execute(invert_ignore_none, pure=True)(
        ...
        for ivis, sub_vis_list in enumerate(vis_list):
            ...
            for sub_sub_vis_list in sub_sub_vis_lists:
                ...
                vis_results.append(
                    rsexecute.execute(gather_image_iteration_results, nout=1)(
...
def deconvolve_list_rsexecute_workflow(
    ...
    psf_list_trimmed = [
        rsexecute.execute(extract_psf)(p, deconvolve_facets) for p in psf_list_trimmed
    ]
    psf_centre = rsexecute.execute(image_gather_channels, nout=1)(
    ...
    if mask is None:
        scattered_results_list = [
            rsexecute.execute(deconvolve, nout=1)(
                d, psf_centre, m, facet, global_threshold
            )
            for d, m, facet in zip(
                scattered_facets_dirty_list, scattered_facets_model_list, facet_list
            )
        ]
    else:
        ...
        scattered_results_list = [
            rsexecute.execute(deconvolve, nout=1)(
                d, psf_centre, m, facet, global_threshold, msk
            )
    ...
    scattered_channel_results_list = [
        rsexecute.execute(image_scatter_channels, nout=nchan)(scat, subimages=nchan)
    ...
    result_list = [
        rsexecute.execute(image_gather_facets, nout=1)(
            [
                scattered_channel_results_list[facet][chan]
                for facet in range(deconvolve_number_facets)
            ],
...
def deconvolve_list_channel_rsexecute_workflow(
    ...
    result = rsexecute.execute(image_gather_channels, nout=1, pure=True)(
        results, output, subimages=subimages
    )
...
def weight_list_rsexecute_workflow(
    ...
    if gcfcf is None:
        gcfcf = [
            rsexecute.execute(create_pswf_convolutionfunction)(m, oversampling=1)
            for m in model_imagelist
        ]
    ...
    weight_list = [
        rsexecute.execute(grid_wt, pure=True, nout=1)(
            vis_list[i], model_imagelist[i], gcfcf
        )
            ...
            if vis is not None:
                ...
                if isinstance(vis, BlockVisibility):
                    vis = griddata_blockvisibility_reweight(
                        vis, agd, g[0][1], weighting=weighting, robustness=robustness
                    )
                else:
