...
def apply_beam_to_skycomponent(
    sc: Union[Skycomponent, List[Skycomponent]], beam: Image
...
def apply_voltage_pattern_to_skycomponent(
    sc: Union[Skycomponent, List[Skycomponent]], vp: Image, inverse=False
) -> Union[Skycomponent, List[Skycomponent]]:
    ...
    for icomp, comp in enumerate(sc):
        ...
        comp_flux_cstokes = convert_pol_frame(
            comp.flux, comp.polarisation_frame, vp.polarisation_frame
        ).reshape([nchan, 2, 2])
