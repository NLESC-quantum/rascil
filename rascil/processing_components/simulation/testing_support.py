...
def create_test_image(
    ...
    if canonical:
        ...
        im = replicate_image(
            im, frequency=frequency, polarisation_frame=im.polarisation_frame
        )
...
def replicate_image(
    ...
    if len(im.data.shape) == 2:
        ...
        newwcs.wcs.crpix = [
            im.wcs.wcs.crpix[0] + 1.0,
            im.wcs.wcs.crpix[1] + 1.0,
            1.0,
            1.0,
        ]
    ...
    for time in times:
        ...
        if phase_error > 0.0 or amplitude_error > 0.0:
            ...
            gt = simulate_gaintable(
                gt=gt, phase_error=phase_error, amplitude_error=amplitude_error
            )
...
def simulate_gaintable(
    ...
    if phase_error > 0.0:
        ...
        for time in range(ntimes):
            for ant in range(nant):
                phase = numpy.random.normal(
                    0, phase_error, nchan + int(smooth_channels) - 1
                )
    ...
    if amplitude_error > 0.0:
        ...
        for time in range(ntimes):
            for ant in range(nant):
                amp = numpy.random.lognormal(
                    mean=0.0,
                    sigma=amplitude_error,
                    size=nchan + int(smooth_channels) - 1,
                )
                if smooth_channels > 1:
    ...
    if nrec > 1:
        if leakage > 0.0:
            leak = numpy.random.normal(
                0, leakage, gt.data["gain"][..., 0, 0].shape
            ) + 1j * numpy.random.normal(0, leakage, gt.data["gain"][..., 0, 0].shape)
            gt.data["gain"][..., 0, 1] = gt.data["gain"][..., 0, 0] * leak
            leak = numpy.random.normal(
                0, leakage, gt.data["gain"][..., 1, 1].shape
            ) + 1j * numpy.random.normal(0, leakage, gt.data["gain"][..., 1, 1].shape)
...
def create_unittest_components(
    ...
    for center in centers:
        ...
        p = int(
            round(rpix[0] + ix * spacing_pixels * numpy.sign(model.wcs.wcs.cdelt[0]))
        ), int(
            round(rpix[1] + iy * spacing_pixels * numpy.sign(model.wcs.wcs.cdelt[1]))
        )
...
def insert_unittest_errors(
        vt, calibration_context="TG", amp_errors=None, phase_errors=None
):
