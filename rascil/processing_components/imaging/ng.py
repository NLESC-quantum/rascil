...
try:
    ...
    def predict_ng(bvis: BlockVisibility, model: Image, **kwargs) -> BlockVisibility:
        ...
        vis_to_im = numpy.round(model.wcs.sub([4]).wcs_world2pix(freq, 0)[0]).astype(
            "int"
        )
        ...
        vis = convert_pol_frame(
            vist.T, model.polarisation_frame, bvis.polarisation_frame, polaxis=2
        )
    ...
    def invert_ng(
        ...
        **kwargs
        ...
        ms = convert_pol_frame(
            ms, bvis.polarisation_frame, im.polarisation_frame, polaxis=2
        )
