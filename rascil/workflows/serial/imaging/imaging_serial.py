    ...
    if facets == 1:
        for ivis, sub_vis_list in enumerate(vis_list):
            ...
            for rows in vis_iter(sub_vis_list, vis_slices):
                ...
                if result is not None:
                    result_image.data += (
                        result[1][:, :, numpy.newaxis, numpy.newaxis] * result[0].data
                    )
        ...
        for ivis, sub_vis_list in enumerate(vis_list):
            ...
            facet_lists = image_scatter_facets(
                template_model_imagelist[ivis], facets=facets
            )
            # Create the graph to divide the visibility into slices. This is by copy.
            sub_sub_vis_lists = visibility_scatter(
...
def restore_list_serial_workflow(
    ...
    **kwargs
    ...
    if len(residual_imagelist) > 0:
        ...
        return [
            restore_cube(model_imagelist[i], psf_imagelist[i][0], **kwargs)
            for i, _ in enumerate(model_imagelist)
        ]
