...
def ical_skymodel_list_rsexecute_workflow(
    ...
    **kwargs
    ...
    if do_selfcal:
        ...
        skymodel_list = [
            rsexecute.execute(set_image)(skymodel_list[i], model_imagelist[i])
    ...
    if nmajor > 1:
        for cycle in range(nmajor):
            if do_selfcal:
                ...
                skymodel_list = [
                    rsexecute.execute(set_image)(
                        skymodel_list[i], deconvolve_model_imagelist[i]
                    )
                    for i in range(len(skymodel_list))
                ]
    ...
    skymodel_list = [
        rsexecute.execute(set_image)(skymodel_list[i], deconvolve_model_imagelist[i])
        for i in range(len(skymodel_list))
    ]
    residual_imagelist = residual_skymodel_list_rsexecute_workflow(
        cal_vis_list,
