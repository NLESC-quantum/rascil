...
def display_ms_as_image(msname, nch=None):
    ...
    for i in range(baselength.shape[0]):
        ...
        iyl = int(np.rint((baselength_log10[i] - blmin_log10) / (blmax_log10 - blmin_log10) * (ny - 1)))
    ...
    for col in range(2):
        for row in range(2):
            ax = axs[row, col]
            idx = 2 * col + row
            im = ax.imshow(visdata_log[:, :, idx], extent=[blmin_log10, blmax_log10, tmax, 0], cmap="nipy_spectral")
