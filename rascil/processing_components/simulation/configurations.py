...
def create_configuration_from_file(
    ...
    antxyz, diameters, anames, mounts = limit_rmax(
...
def create_configuration_from_SKAfile(
    ...
    assert antdiamlonglat.shape[1] == 3, (
        "Antenna array has wrong shape %s" % antdiamlonglat.shape
    )
...
def limit_rmax(antxyz, diameters, names, mounts, rmax):
    ...
    if rmax is not None:
        ...
        log.debug(
            "create_configuration_from_file: %d antennas/stations" % (antxyz.shape[0])
        )
...
def create_LOFAR_configuration(antfile: str, location, rmax=1e6) -> Configuration:
    ...
    antxyz, diameters, mounts, anames = limit_rmax(
        antxyz, diameters, anames, mounts, rmax
    )
...
def create_named_configuration(name: str = "LOWBD2", **kwargs) -> Configuration:
    ...
    meerkat_location = EarthLocation(
        lon=21.44388889 * u.deg, lat=-30.7110565 * u.deg, height=1086.6
    )
    if name == "LOWBD2":
        ...
        fc = create_configuration_from_file(
            antfile=rascil_data_path("configurations/LOWBD2.csv"),
            location=location,
            mount="xy",
            names="LOWBD2_%d",
            vp_type="LOW",
        ...
        fc = create_configuration_from_file(
            antfile=rascil_data_path("configurations/LOWBD2-CORE.csv"),
            vp_type="LOW",
            location=location,
            mount="xy",
            names="LOWBD2_%d",
        ...
        fc = create_configuration_from_MIDfile(
            antfile=rascil_data_path("configurations/ska1low_local.cfg"),
            vp_type="LOW",
            mount="xy",
            name=name,
            location=location,
        ...
        fc = create_configuration_from_MIDfile(
            antfile=rascil_data_path("configurations/ska1mid_local.cfg"),
        ...
        fc = create_configuration_from_MIDfile(
            antfile=rascil_data_path("configurations/mkatplus_local.cfg"),
        ...
        location = EarthLocation(
            lon=+116.6356824 * u.deg, lat=-26.7013006 * u.deg, height=377.0
        )
        log.debug(
            "create_named_configuration: %s\n\t%s\n\t%s"
            % (name, location.geocentric, location.geodetic)
        ...
        fc = create_configuration_from_MIDfile(
            antfile=rascil_data_path("configurations/askap_local.cfg"),
            vp_type="ASKAP",
            mount="equatorial",
            name=name,
            location=location,
        ...
        fc = create_configuration_from_MIDfile(
            antfile=rascil_data_path("configurations/lofar_local.cfg"),
            location=location,
            mount="xy",
            vp_type="LOFAR",
            name=name,
        ...
        location = EarthLocation(
            lon=-107.6184 * u.deg, lat=34.0784 * u.deg, height=2124.0
        )
        log.debug(
            "create_named_configuration: %s\n\t%s\n\t%s"
            % (name, location.geocentric, location.geodetic)
        ...
        fc = create_configuration_from_file(
            antfile=rascil_data_path("configurations/vlaa_local.csv"),
            location=location,
            mount="azel",
            names="VLA_%d",
            vp_type="VLA",
