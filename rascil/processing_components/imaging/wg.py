...
try:
    ...
    wagg = None
    print("WAGG module is not imported")
...
def predict_wg(bvis: BlockVisibility, model: Image, **kwargs) -> BlockVisibility:
    """Predict using convolutional degridding.

    Nifty-gridder GPU version. https://gitlab.com/ska-telescope/sdp/ska-gridder-nifty-cuda
    ...
    if mfs:
        for vpol in range(vnpol):
            # vis1 = wagg.dirty2ms(uvw, freqs, image, weights, pixsize_rad, pixsize_rad,
            # 		1e-10, do_wstacking)
            #            model["pixels"].data[0, vpol, :, :] = numpy.flipud(model["pixels"].data[0, vpol, :, :])
            vist[vpol, :, :] = wg.dirty2ms(
                fuvw.astype(float),
                bvis.frequency.data.astype(float),
                model["pixels"].data[0, vpol, :, :].T.astype(float),
                # model["pixels"].data[0, vpol, :, :].astype(float),
                None,
                pixsize,
                pixsize,
                #                0,
        ...
        for vpol in range(vnpol):
            for vchan in range(vnchan):
                ...
                #               model["pixels"].data[imchan, vpol, :, :] = numpy.flipud(model["pixels"].data[imchan, vpol, :, :])
                vist[vpol, vchan, :] = wg.dirty2ms(
                    fuvw.astype(float),
                    numpy.array(freq[vchan : vchan + 1]).astype(float),
                    model["pixels"].data[imchan, vpol, :, :].T.astype(float),
                    # model["pixels"].data[imchan, vpol, :, :].astype(float),
                    None,
                    pixsize,
                    pixsize,
                    #                    0,
...
def invert_wg(
    ...
    **kwargs
    ...
    In the imaging and pipeline workflows, this may be invoked using context='wg'. It is the default
    ...
    if mfs:
        for pol in range(npol):
            ...
            if numpy.max(numpy.abs(lms)) > 0.0:
                ...
                #                wagg.ms2dirty(uvw, freqs, vis, weights, image_size, image_size,
                #                      pixsize_rad, pixsize_rad, 1e-10, do_wstacking)
                dirty = wg.ms2dirty(
                    fuvw,
                    bvis.frequency.data,
                    lms,
                ...
                # dirty = numpy.flipud(dirty)
                # im["pixels"].data[0, pol] += dirty
        ...
        for pol in range(npol):
            for vchan in range(vnchan):
                ...
                if numpy.max(numpy.abs(lms)) > 0.0:
                    ...
                    # dirty = numpy.flipud(dirty)
                    # im["pixels"].data[ichan, pol] += dirty
