...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ical_nmajor", type=int, default=10, help="Number of major cycles for ICAL"
    )
    parser.add_argument(
        "--mpccal_nmajor",
        type=int,
    ...
    parser.add_argument(
        "--context", type=str, default="2d", help="Imaging context: 2d|timeslice|wstack"
    )
    parser.add_argument(
        "--rmax", type=float, default=2500.0, help="Maximum baseline (m)"
    )
    parser.add_argument(
        "--flux_limit", type=float, default=0.2, help="Flux limit for GLEAM components"
    )
    parser.add_argument(
        "--finding_threshold",
        type=float,
    ...
    parser.add_argument(
        "--ninitial", type=int, default=1, help="Number of initial components to use"
    )
    ...
    if args.use_dask:
        rsexecute.set_client(
            n_workers=args.nworkers, memory_limit=args.memory * 1024 * 1024 * 1024
        )
    else:
    ...
    distance = numpy.hypot(
        low.xyz[:, 0] - centre[0], low.xyz[:, 1] - centre[1], low.xyz[:, 2] - centre[2]
    )
    ...
    all_components = sorted(
        all_components, key=lambda comp: numpy.max(comp.flux), reverse=True
    )
    print("Number of components in simulation %d" % len(all_components))

    all_gaintables = create_gaintable_from_screen(
        block_vis, all_components, rascil_data_path("models/test_mpc_screen.fits")
    )
    ...
    ical_skymodel = [
        SkyModel(
            components=all_components[: args.ninitial],
    ...
    psf_obs = invert_list_rsexecute_workflow(
        [future_vis], [future_model], context="2d", dopsf=True
    )
    ...
    export_image_to_fits(
        ical_restored,
        rascil_path("test_results/low-sims-mpc-ical-restored_%.1frmax.fits" % rmax),
    )
    ...
    ical_components = find_skycomponents(
        ical_restored, fwhm=2, threshold=args.finding_threshold, npixels=12
    )
    ...
    ical_components = sorted(
        ical_components, key=lambda comp: numpy.max(comp.flux), reverse=True
    )
    ...
    mpccal_skymodel = initialize_skymodel_voronoi(
        model, ical_components, ical_skymodel[0].gaintable
    )
    ...
    result = restore_list_rsexecute_workflow(
        [mpccal_combined_model], psf_obs, [(mpccal_residual, 0.0)]
    )
    result = rsexecute.compute(result, sync=True)
    mpccal_restored = result[0]

    mpccal_components = find_skycomponents(
        mpccal_restored, fwhm=2, threshold=args.finding_threshold, npixels=12
    )
    mpccal_components = sorted(
        mpccal_components, key=lambda comp: numpy.max(comp.flux), reverse=True
    )
    ...
    show_image(
        mpccal_restored,
        title="MPCCAL restored image",
        components=mpccal_components,
        vmax=0.3,
        vmin=-0.03,
    )
    plt.show(block=block_plots)
    export_image_to_fits(
        mpccal_restored,
        rascil_path("test_results/low-sims-mpc-mpccal-restored_%.1frmax.fits" % rmax),
    )

    mpccal_fluxes = [comp.flux[0, 0] for comp in mpccal_components]
    plt.clf()
    plt.semilogy(
        numpy.arange(len(all_fluxes)), all_fluxes, marker=".", label="Original"
    )
    plt.semilogy(numpy.arange(len(ical_fluxes)), ical_fluxes, marker=".", label="ICAL")
    plt.semilogy(
        numpy.arange(len(mpccal_fluxes)), mpccal_fluxes, marker=".", label="MPCCAL"
    )
    plt.title("All component fluxes")
    plt.ylabel("Flux (Jy)")
    plt.legend()
    ...
    show_image(
        difference_image, title="MPCCAL - ICAL image", components=ical_components
    )
    plt.show(block=block_plots)
    export_image_to_fits(
        difference_image,
        rascil_path(
            "test_results/low-sims-mpc-mpccal-ical-restored_%.1frmax.fits" % rmax
        ),
    )
    ...
    export_image_to_fits(
        newscreen,
        rascil_path("test_results/low-sims-mpc-mpccal-screen_%.1frmax.fits" % rmax),
    )
    export_image_to_fits(
        weights,
        rascil_path(
