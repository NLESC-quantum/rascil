...
if __name__ == "__main__":
    ...
    vt = create_blockvisibility_from_ms(
        rascil_data_path("vis/sim-2.ms"), start_chan=35, end_chan=39
    )[0]
    ...
    export_image_to_fits(
        restored, "%s/rascil_imaging_sim_2_restored.fits" % (results_dir)
    )
    export_image_to_fits(
        residual, "%s/rascil_imaging_sim_2_residual.fits" % (results_dir)
    )
