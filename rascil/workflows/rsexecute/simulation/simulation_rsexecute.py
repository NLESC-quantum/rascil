...
def calculate_residual_from_gaintables_rsexecute_workflow(
    bvis_list, components, model_list, nominal_gtl, actual_gtl, **kwargs
):
    ...
    :return:
    """
    actual_dirty_list = calculate_residual_dft_rsexecute_workflow(
        bvis_list, components, model_list, actual_gtl
    )
    nominal_dirty_list = calculate_residual_dft_rsexecute_workflow(
...
def calculate_residual_fft_rsexecute_workflow(
    ...
    :param sub_components: List of components (or graph)
    :param sub_model_list: List of models (or graph)
    :param nominal_gt_list: List of gaintables for no error (or graph)
    :param context: Imaging context e.g. '2d' or 'ng'
    :param residual: Calculate residual visibility (True)
    :return:
    """
...
def calculate_selfcal_residual_from_gaintables_rsexecute_workflow(
    ...
    **kwargs
    ...
    :param sub_model_list: List of models (or graph)
    :param nominal_gt_list: List of gaintables for no error (or graph)
    :param actual_gt_list: List of gaintables for error (or graph)
    :param context: Imaging context e.g. '2d' or 'ng'
    :param residual: Calculate residual visibility (True)
    ...
    actual_sm_list = [
        [
            rsexecute.execute(SkyModel, nout=1)(
                components=[sub_components[i]], gaintable=actual_gt_list[ibv][i]
            )
    ...
    nominal_sm_list = [
        [
            rsexecute.execute(SkyModel, nout=1)(
                components=[sub_components[i]], gaintable=nominal_gt_list[ibv][i]
            )
            for i, _ in enumerate(sub_components)
        ]
        for ibv, bv in enumerate(sub_bvis_list)
    ...
    nominal_bvis_list = [
        predict_skymodel_list_rsexecute_workflow(
            nominal_bvis_list[ibv],
            nominal_sm_list[ibv],
            context=context,
            docal=True,
            **kwargs
        )
        for ibv, bvis in enumerate(nominal_bvis_list)
    ]
    ...
    actual_bvis_list = [
        predict_skymodel_list_rsexecute_workflow(
            actual_bvis_list[ibv],
            actual_sm_list[ibv],
            context=context,
            docal=True,
            **kwargs
        )
        for ibv, bvis in enumerate(actual_bvis_list)
    ]
    ...
    def selfcal_convert(actual_bvis, nominal_bvis):
        if selfcal:
            ...
            actual_bvis = apply_gaintable(actual_bvis, gt)
        if residual:
            actual_bvis["vis"].data = actual_bvis["vis"].data - nominal_bvis["vis"].data
        return actual_bvis
    ...
    dirty_list = invert_list_rsexecute_workflow(
        actual_vis_list, sub_model_list, context=context, **kwargs
    )
    return dirty_list
