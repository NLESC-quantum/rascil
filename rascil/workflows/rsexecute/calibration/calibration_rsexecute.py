...
def calibrate_list_rsexecute_workflow(
    ...
    **kwargs
    ...
    if global_solution and (len(vis_list) > 1):
        ...
        point_vislist = [
            rsexecute.execute(convert_visibility_to_blockvisibility, nout=1)(v)
            for v in vis_list
        ]
        point_modelvislist = [
            rsexecute.execute(convert_visibility_to_blockvisibility, nout=1)(mv)
            for mv in model_vislist
        ]
        point_vislist = [
            rsexecute.execute(divide_visibility, nout=1)(
                point_vislist[i], point_modelvislist[i]
            )
            for i, _ in enumerate(point_vislist)
        ]

        global_point_vis_list = rsexecute.execute(visibility_gather_channel, nout=1)(
            point_vislist
        )
