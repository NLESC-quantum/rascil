...
def bdsf_qa_image(im_data):
    ...
    Set of statistics of an image; max, min, maxabs, rms, sum, medianabs, medianabsdevmedian, median
...
def ci_checker_diagnostics(img):
    """
    Log and plot diagnostics from the ci_checker.
    ...
    im_data = img.resid_gaus_arr

    image_stats = bdsf_qa_image(im_data)

    n, bins, _ = plt.hist(im_data.ravel(), bins=1000, density=False)
    ...
    fitted_model = fitter(model, mid_points, n)

    plt.plot(mid_points, fitted_model(mid_points))
    plt.yscale("symlog")
    ...
    plt.savefig(diagnostics_dir + "/hist_continuum_imaging_residuals.png")
    plt.close()
