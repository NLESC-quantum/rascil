        ...
        for fy in range(facets):
            ...
            for fx in range(facets):
                ...
                if (x >= 0) and (x + dx) <= nx and (y >= 0) and (y + dy) <= ny:
                    ...
                    subim = create_image_from_array(im["pixels"].data.values[..., y:y + dy, x:x + dx], wcs, im.polarisation_frame)
...
def image_channel_iter(im: Image, subimages=1) -> collections.abc.Iterable:
    ...
    for i, channel in enumerate(channels):
        ...
        # Yield image from slice (reference!)
        yield create_image_from_array(im["pixels"].data.values[channel:channel_max, ...], wcs, im.polarisation_frame)
