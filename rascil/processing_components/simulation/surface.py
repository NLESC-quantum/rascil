                    ...
                    try:
                        ...
                        for ivp, _ in enumerate(vp_types):
                            for gchan in range(gnchan):
                                ...
                                pixloc = (
                                    vp[ivp].wcs.deepcopy().wcs_world2pix(worldloc, 0)[0]
                                )
                                assert pixloc[0] > 2
...
def simulate_gaintable_from_zernikes(
    ...
    **kwargs
    ...
    for ivp, vp in enumerate(vp_list):
        ...
        real_splines.append(
            RectBivariateSpline(
                range(ny), range(nx), vp.data[0, 0, ...].real, kx=order, ky=order
            )
        )
        imag_splines.append(
            RectBivariateSpline(
                range(ny), range(nx), vp.data[0, 0, ...].imag, kx=order, ky=order
            )
        )
