...
__all__ = [
    "calculate_noise_blockvisibility",
...
def addnoise_visibility(vis, t_sys=None, eta=None):
    ...
    if isinstance(vis, Visibility):
        sigma = calculate_noise_visibility(
            vis.data["channel_bandwidth"],
            vis.data["integration_time"],
            vis.configuration.diameter[0],
            t_sys=t_sys,
            eta=eta,
        )
            ...
            for ant1 in range(vis.nants):
                for ant2 in range(ant1, vis.nants):
                    for pol in range(vis.npol):
                        vis.data["vis"][
                            row, ant2, ant1, :, pol
                        ].real += numpy.random.normal(0, sigma[row, ...])
                        vis.data["vis"][
                            row, ant2, ant1, :, pol
                        ].imag += numpy.random.normal(0, sigma[row, ...])
                        vis.data["vis"][row, ant1, ant2, :, pol] = numpy.conjugate(
                            vis.data["vis"][row, ant2, ant1, :, pol]
                        )
