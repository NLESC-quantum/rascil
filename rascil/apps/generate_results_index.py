...
def _find_path_for_index_files(path_to_check):
    """
    When the code is run from a Docker container,
    we want to make sure the output index files point
    to files on the local machine, not on the container.
    For this, DOCKER_PATH has to be set to the local path
    of directory we mount to the container.

    :param path_to_check: file path that is used to determine where file lives

    :return: if DOCKER_PATH is not set, return input path,
             else replace the root directory of the input path, with DOCKER_PATH
    """
    try:
    ...
    if not docker_path:
        ...
        dir_to_replace = "/" + path_to_check[1:].split("/")[0]
        new_path = path_to_check.replace(dir_to_replace, docker_path)
        return new_path
...
def generate_html_file(path, sorted_file_dict):
    ...
    docker_to_local_path = _find_path_for_index_files(path)
    ...
    LOGGER.info("HTML file created at: %s", docker_to_local_path + "/index.html")
...
def generate_markdown_file(path, sorted_file_dict):
    ...
    LOGGER.info("Markdown file created at: %s", docker_to_local_path + "/index.md")
