...
def shift_vis_to_image(
    ...
    assert isinstance(vis, Visibility) or isinstance(vis, BlockVisibility), (
        "vis is not a Visibility or " "BlockVisibility: %r" % vis
    )
...
def predict_2d(
    vis: Union[BlockVisibility, Visibility], model: Image, gcfcf=None, **kwargs
...
def create_image_from_visibility(
    vis: Union[BlockVisibility, Visibility], **kwargs
) -> Image:
    ...
    channel_bandwidth = (
        get_parameter(
            kwargs, "channel_bandwidth", 0.99999999999 * vis.channel_bandwidth[0]
        )
        * units.Hz
    )
    ...
    if (inchan == vnchan) and vnchan > 1:
        ...
        assert (
            numpy.abs(channel_bandwidth.value) > 0.0
        ), "Channel width must be non-zero for mfs mode"
        log.debug(
...
def advise_wide_field(
    vis: Union[BlockVisibility, Visibility],
    delA=0.02,
    ...
    freq_sampling_primary_beam = numpy.max(vis.frequency) * (
    ...
    if verbose:
        log.info(
            "advice_wide_field: (vis_slices) Number of planes in w stack %d (primary beam)"
            % (vis_slices)
        )
        log.info(
            "advice_wide_field: (wprojection_planes) Number of planes in w projection %d (primary beam)"
            % (wprojection_planes)
        )
    ...
    if verbose:
        log.info(
            "advice_wide_field: (nwpixels) W support = %d (pixels) (primary beam)"
            % nwpixels
        )
