...
def create_griddata_from_array(
    data: numpy.array, grid_wcs: WCS, polarisation_frame: PolarisationFrame
) -> GridData:
    ...
    log.debug(
        "create_griddata_from_array: created %s image of shape %s"
        % (data.dtype, str(data.shape))
    )

    return GridData(
        data=data, polarisation_frame=polarisation_frame, grid_wcs=grid_wcs.deepcopy()
    )
...
def qa_griddata(gd, context="") -> QA:
    ...
    data = {
        "shape": str(gd["pixels"].data.shape),
