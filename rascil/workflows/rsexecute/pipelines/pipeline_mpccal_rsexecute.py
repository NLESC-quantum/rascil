...
from rascil.processing_components.skymodel import (
...
def mpccal_skymodel_list_rsexecute_workflow(
    visobs, model, theta_list, nmajor=10, context="2d", mpccal_progress=None, **kwargs
):
    ...
    psf_obs = invert_list_rsexecute_workflow(
        [visobs], [model], context=context, dopsf=True
    )
    ...
    for iteration in range(nmajor):
        ...
        vdatamodel_list = predict_skymodel_list_rsexecute_workflow(
            visobs, theta_list, context=context, docal=True, **kwargs
        )
        vdatamodel_list = crosssubtract_datamodels_skymodel_list_rsexecute_workflow(
        ...
        dirty_all_conv = convolve_skymodel_list_rsexecute_workflow(
            visobs, theta_list, context=context, docal=True, **kwargs
        )
        dirty_all_cal = invert_skymodel_list_rsexecute_workflow(
        ...
        def pipeline_mpc_make_residual(dcal, tl, it):
            ...
            for i, d in enumerate(dcal):
                assert (
                    numpy.max(numpy.abs(d[0]["pixels"].data)) > 0.0
                ), "Residual subimage is zero"
                if tl[i].mask is None:
                    res["pixels"].data += d[0]["pixels"].data
                else:
                    assert (
                        numpy.max(numpy.abs(tl[i].mask["pixels"].data)) > 0.0
                    ), "Mask image is zero"
                    res["pixels"].data += (
                        d[0]["pixels"].data * tl[i].mask["pixels"].data
                    )

            assert (
                numpy.max(numpy.abs(res["pixels"].data)) > 0.0
            ), "Residual image is zero"
        ...
        vpredicted_list = predict_skymodel_list_rsexecute_workflow(
            visobs, theta_list, context=context, docal=True, **kwargs
        )
        vcalibrated_list, gaintable_list = calibrate_list_rsexecute_workflow(
            vdatamodel_list,
            vpredicted_list,
        ...
        if mpccal_progress is not None:
            theta_list = rsexecute.execute(mpccal_progress, nout=len(theta_list))(
                residual, theta_list, gaintable_list, iteration
            )
        theta_list = rsexecute.execute(
            update_skymodel_from_image, nout=len(theta_list)
        )(theta_list, deconvolved[0])

        theta_list = rsexecute.execute(
            update_skymodel_from_gaintables, nout=len(theta_list)
        )(theta_list, gaintable_list, calibration_context="T")
