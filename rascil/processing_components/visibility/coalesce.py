...
def coalesce_visibility(
    ...
    log.debug(
        "coalesce_visibility: Maximum coalescence (t,f) = (%d, %d)"
        % (max_time_coal, max_frequency_coal)
    )
    log.debug(
        "coalesce_visibility: Original %s, coalesced %s"
...
def convert_blockvisibility_to_visibility(vis: BlockVisibility) -> Visibility:
    ...
    log.debug(
        "convert_visibility: Original %s, converted %s"
...
def decoalesce_visibility(vis: Visibility) -> BlockVisibility:
    ...
    assert isinstance(vis.blockvis, BlockVisibility), (
            "No blockvisibility in vis %r" % vis
    )
    ...
    assert (
            numpy.max(vis.cindex) < vis.vis.shape[0]
    ), "Incorrect template used in decoalescing"
    ...
    log.debug(
        "decoalesce_visibility: Coalesced %s, decoalesced %s"
...
def average_in_blocks(
    ...
    mask = numpy.where(uvdist_max > 0.0)
    mask0 = numpy.where(uvdist_max <= 0.0)
    ...
    numpy.putmask(
        frequency_average, frequency_average > max_frequency_coal, max_frequency_coal
    )
    ...
    for a2 in ua:
        for a1 in ua:
            if (time_average[a2, a1] > 0) & (
                    frequency_average[a2, a1]
                    > 0 & (allpwtsgrid[:, a2, a1, ...].any() > 0.0)
            ):
                time_chunks, _ = average_chunks(
                    times, allcpwtsgrid[:, a2, a1], time_average[a2, a1]
                )
                time_chunk_len[a2, a1] = time_chunks.shape[0]
                frequency_chunks, _ = average_chunks(
                    frequency, alltpwtsgrid[a2, a1, :], frequency_average[a2, a1]
                )
    ...
    for a2 in ua:
        for a1 in ua:
            ...
            for axis in range(3):
                uvwgrid = numpy.outer(
                    uvw[:, a2, a1, axis], frequency / constants.c.value
                )
    ...
    assert (
            cnvis == visstart
    ), "Mismatch between number of rows in coalesced visibility %d and index %d" % (
        cnvis,
        visstart,
    )
...
def convert_blocks(
    ...
    rowgrid_mask = numpy.argwhere(mask_rowgrid is True)
