...
def append_visibility(
    ...
    assert (
        vis.polarisation_frame == othervis.polarisation_frame
    ), "Polarisation frames differ"
    assert (
        abs(vis.phasecentre.ra.value - othervis.phasecentre.ra.value) < 1e-15
    ), "RAs differ"
    assert (
        abs(vis.phasecentre.dec.value - othervis.phasecentre.dec.value) < 1e-15
    ), "Declinations differ"
    assert (
        vis.phasecentre.separation(othervis.phasecentre).value < 1e-15
    ), "Phasecentres differ"
            ...
            for ant1 in range(vis.nants):
                for pol in range(vis.polarisation_frame.npol):
                    ...
                    if mask is not None:
                        wt[mask] = 0.0
                    fit = numpy.polyfit(
                        x, vis.data["vis"][row, ant2, ant1, :, pol], w=wt, deg=degree
                    )
...
def average_blockvisibility_by_channel(
    ...
    for group in channels:
        ...
        vfvw = (
            vis.flagged_vis[..., group[0] : group[1], :]
        ...
        mask = newvis.flagged_weight > 0.0
        newvis.data["vis"][mask] = (
            newvis.data["vis"][mask] / newvis.flagged_weight[mask]
        )
...
def convert_blockvisibility_to_stokes(vis):
    ...
    if poldef == PolarisationFrame("linear"):
        ...
        vis.data["flags"] = numpy.logical_or(vis.flags[..., 0], vis.flags[..., 3])[
            ..., numpy.newaxis
        ]
        vis.polarisation_frame = PolarisationFrame("stokesIQUV")
    elif poldef == PolarisationFrame("circular"):
...
def convert_visibility_to_stokesI(vis):
    ...
    if poldef == PolarisationFrame("linear"):
        ...
        vis_imaging_weight = (
            vis.flagged_imaging_weight[..., 0] + vis.flagged_imaging_weight[..., 3]
        )[..., numpy.newaxis]
    elif poldef == PolarisationFrame("circular"):
...
def convert_blockvisibility_to_stokesI(vis):
    ...
    if poldef == PolarisationFrame("linear"):
        ...
        vis_imaging_weight = (
            vis.flagged_imaging_weight[..., 0] + vis.flagged_imaging_weight[..., 1]
        )[..., numpy.newaxis]
    elif poldef == PolarisationFrame("circular"):
        vis_data = convert_circular_to_stokesI(vis.data["vis"])
        vis_flags = numpy.logical_or(vis.flags[..., 0], vis.flags[..., 3])[
