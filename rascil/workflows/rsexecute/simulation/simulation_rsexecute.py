...
def calculate_residual_from_gaintables_rsexecute_workflow(
        bvis_list, components, model_list, no_error_gtl, error_gtl
):
    ...
    :return:
    """
    error_dirty_list = calculate_residual_dft_rsexecute_workflow(
        bvis_list, components, model_list, error_gtl
    )
    no_error_dirty_list = calculate_residual_dft_rsexecute_workflow(
        bvis_list, components, model_list, no_error_gtl
    )
    ...
    residual_list = rsexecute.execute(subtract, nout=1)(
        error_dirty_list, no_error_dirty_list
    )

    return residual_list


def predict_fft_components_rsexecute_workflow(
        sub_bvis_list,
        sub_components,
        sub_model_list,
        vp_list,
    ...
    """
    fft_model_list = [
        rsexecute.execute(create_empty_image_like)(m) for m in sub_model_list
    ]
    fft_model_list = [
        rsexecute.execute(insert_skycomponent)(
            m, sc=sub_components, insert_method=insert_method
        )
        for m in fft_model_list
    ]
    fft_model_list = [
        rsexecute.execute(apply_voltage_pattern_to_image)(m, vp_list[im])
    ...
    fft_bvis_list = predict_list_rsexecute_workflow(
        fft_bvis_list, fft_model_list, context=context, **kwargs
    )
    return fft_bvis_list


def predict_fft_image_rsexecute_workflow(
        sub_bvis_list, sub_model_list, vp_list, context="2d", **kwargs
):
    """Calculate residual image corresponding to a set of voltage patternss
...
def calculate_residual_fft_rsexecute_workflow(
        sub_bvis_list, sub_components, sub_model_list, vp_list, context="2d", **kwargs
):
    ...
    :return:
    """
    fft_bvis_list = predict_fft_components_rsexecute_workflow(
        sub_bvis_list,
        sub_components,
        sub_model_list,
        vp_list,
    ...
    return sum_invert_results_rsexecute(
        invert_list_rsexecute_workflow(
            fft_bvis_list, sub_model_list, context=context, **kwargs
        )
    )


def predict_dft_rsexecute_workflow(
        sub_bvis_list, sub_components, gt_list, context="2d"
):
    """Calculate residual image corresponding to a set of gaintables
    ...
    dft_sm_list = [
        [
            rsexecute.execute(SkyModel, nout=1)(
                components=[sub_components[icomp]], gaintable=gt_list[ibv][icomp]
            )
    ...
    dft_bvis_list = [
        predict_skymodel_list_compsonly_rsexecute_workflow(
            dft_bvis_list[ibv], dft_sm_list[ibv], docal=True
        )
    ...
    dft_bvis_list = [
        sum_predict_results_rsexecute(
            [dft_bvis_list[ivis][icomp] for icomp, _ in enumerate(sub_components)]
        )
...
def calculate_residual_dft_rsexecute_workflow(
        sub_bvis_list, sub_components, sub_model_list, gt_list, context="2d", **kwargs
):
    """Calculate residual image corresponding to a set of gaintables
    ...
    dft_bvis_list = predict_dft_rsexecute_workflow(
        sub_bvis_list, sub_components, gt_list, context=context
    )
    return sum_invert_results_rsexecute(
        invert_list_rsexecute_workflow(
...
def calculate_selfcal_residual_from_gaintables_rsexecute_workflow(
    ...
    error_sm_list = [
        [
            rsexecute.execute(SkyModel, nout=1)(
                components=[sub_components[i]], gaintable=error_gt_list[ibv][i]
            )
    ...
    no_error_sm_list = [
        [
            rsexecute.execute(SkyModel, nout=1)(
                components=[sub_components[i]], gaintable=no_error_gt_list[ibv][i]
            )
            for i, _ in enumerate(sub_components)
        ]
        for ibv, bv in enumerate(sub_bvis_list)
    ...
    error_vis_list = [
        rsexecute.execute(selfcal_convert)(
            error_bvis_list[ibv], no_error_bvis_list[ibv]
        )
        for ibv, _ in enumerate(error_bvis_list)
    ]

    dirty_list = invert_list_rsexecute_workflow(
        error_vis_list, sub_model_list, context=context, **kwargs
    )
    return dirty_list
...
def create_pointing_errors_gaintable_rsexecute_workflow(
    ...
    if show:
        ...
        if time_series != "":
            ...
            plot_file = (
                    "pointing_error_dynamic_%.2f_static_(%.2f,%.2f)_global_(%.2f,%.2f).png"
                    % (
                        r2s * pointing_error,
    ...
    if show:
        ...
        if time_series_type != "":
            ...
            r2s = 180 * 3600.0 / numpy.pi
            plot_file = (
                    "gaintable_dynamic_%.2f_static_(%.2f,%.2f)_global_(%.2f,%.2f).png"
                    % (
                        r2s * pointing_error,
                        r2s * static_pointing_error[0],
                        r2s * static_pointing_error[1],
        ...
        plot_gaintable(
            tmp_gt_list,
            title="%s: dish 0 amplitude gain, %s" % (basename, time_series_type),
            plot_file=plot_file,
        )
...
def create_surface_errors_gaintable_rsexecute_workflow(
    ...
    vp_nominal_list = [rsexecute.execute(find_vp_nominal)(band) for _ in sub_bvis_list]
...
def create_polarisation_gaintable_rsexecute_workflow(
        band, sub_bvis_list, sub_components, get_vp, show=True, basename="", normalise=True
):
    ...
    if show:
        ...
        plot_gaintable(
            no_error_gt_list, plot_file=plot_file, title=basename + " nominal"
        )
...
def create_standard_low_simulation_rsexecute_workflow(
    ...
    low = create_configuration_from_MIDfile(
        rascil_data_path("configurations/ska1low_local.cfg"),
        rmax=rmax,
        location=low_location,
    )
