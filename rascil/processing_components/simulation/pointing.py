...
def simulate_gaintable_from_pointingtable(
    ...
    **kwargs
    ...
    real_spline = [
        [
            RectBivariateSpline(
                range(ny), range(nx), vp.data[chan, pol, ...].real, kx=order, ky=order
            )
    ...
    imag_spline = [
        [
            RectBivariateSpline(
                range(ny), range(nx), vp.data[chan, pol, ...].imag, kx=order, ky=order
            )
            for chan in range(nchan)
        ]
        for pol in range(npol)
    ...
    for iha, rows in enumerate(vis_timeslice_iter(vis, vis_slices=vis_slices)):
        ...
        azimuth_centre, elevation_centre = calculate_azel(
            v.configuration.location, utc_time, v.phasecentre
        )
        ...
        for icomp, comp in enumerate(sc):
            ...
            if elevation_centre >= elevation_limit:
                ...
                utc_time = Time(
                    [numpy.average(v.time) / 86400.0], format="mjd", scale="utc"
                )
                ...
                for ant in range(nant):
                    ...
                    az_comp = (
                        azimuth_centre
                        + pointing_ha[0, ant, 0, 0, 0] / numpy.cos(elevation_centre)
                    ) * r2d
                    ...
                    gaintables[icomp].gain[iha, :, :, :] = antgain[:, :, :].reshape(
    ...
    assert (
        number_good > 0
    ), "simulate_gaintable_from_pointingtable: No points inside the voltage pattern image"
    if number_bad > 0:
        log.warning(
...
def simulate_pointingtable(
    ...
    **kwargs
    ...
    if pointing_error > 0.0:
        ...
        pt.data["pointing"] += numpy.random.normal(
            0.0, pointing_error, pt.data["pointing"].shape
        )
    if (abs(static_pointing_error[0]) > 0.0) or (abs(static_pointing_error[1]) > 0.0):
        ...
        static_pe[..., 0] = numpy.random.normal(
            0.0, static_pointing_error[0], static_pe[..., 0].shape
        )[numpy.newaxis, ...]
        static_pe[..., 1] = numpy.random.normal(
...
def simulate_pointingtable_from_timeseries(
    ...
    for axis in axes:
        ...
        for ant in range(nant):
            ...
            if axis == "az":
                pt.data["pointing"][:, ant, :, :, 0] = ts[
                    :ntimes, numpy.newaxis, numpy.newaxis, ...
                ]
            elif axis == "el":
                pt.data["pointing"][:, ant, :, :, 1] = ts[
                    :ntimes, numpy.newaxis, numpy.newaxis, ...
                ]
            elif axis == "pxel":
                pt.data["pointing"][:, ant, :, :, 0] = ts[
