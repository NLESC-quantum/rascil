...
def create_visibility(config: XConfiguration, times: numpy.array, frequency: numpy.array,
    ...
    :param config: XConfiguration of antennas
...
def create_blockvisibility(config: XConfiguration,
                           times: numpy.array,
    ...
    latitude = config.location.geodetic[1].to('rad').value
    ants_xyz = numpy.array(config.data['xyz'])
...
def export_blockvisibility_to_ms(msname, vis_list, source_name=None):
    ...
    for vis in vis_list:
        ...
        n_ant = len(vis.configuration.xyz.values)
