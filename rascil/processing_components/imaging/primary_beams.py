...
def mosaic_pb(model, telescope, pointingcentres, use_local=True):
    ...
    """
    assert isinstance(
        pointingcentres, collections.abc.Iterable
    ), "Need a list of pointing centres"
...
def create_vp_generic_numeric(
    ...
    for chan in range(nchan):
        ...
        if isinstance(zernikes, collections.abc.Iterable):
            ...
            trc = pnx // 2 + ndisk // 2
            for pol in range(npol):
                xfr.data[chan, pol, blc:trc, blc:trc] = xfr.data[
...
def create_low_test_beam(model: Image, use_local=True) -> Image:
    ...
    for chan in range(nchan):
        ...
        beam2d = create_image_from_array(
            beam.data[0, 0, :, :], beam2dwcs, model.polarisation_frame
        )
        reprojected_beam2d, footprint = reproject_image(
            beam2d, model2dwcs, shape=model2dshape
        )
...
def create_low_test_vp(model: Image, use_local=True) -> Image:
    ...
    for chan in range(nchan):
        ...
        beam2d_real = create_image_from_array(
            numpy.real(beam.data[0, 0, :, :]), beam2dwcs, model.polarisation_frame
        )
        beam2d_imag = create_image_from_array(
            numpy.imag(beam.data[0, 0, :, :]), beam2dwcs, model.polarisation_frame
        )
        reprojected_beam2d_real, footprint = reproject_image(
            beam2d_real, model2dwcs, shape=model2dshape
        )
        reprojected_beam2d_imag, footprint = reproject_image(
            beam2d_imag, model2dwcs, shape=model2dshape
        )
        assert numpy.max(footprint.data) > 0.0, "No overlap between beam and model"
        ...
        for pol in range(npol):
            reprojected_beam.data[chan, pol, :, :] = (
                reprojected_beam2d_real.data[:, :]
