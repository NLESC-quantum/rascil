...
__all__ = ['create_xconfiguration_from_file',
           'create_xconfiguration_from_MIDfile',
           'create_xconfiguration_from_SKAfile',
           'create_LOFAR_xconfiguration',
           'create_named_xconfiguration',
           'limit_rmax',
...
def create_xconfiguration_from_file(antfile: str, location: EarthLocation = None,
                                    mount: str = 'azel',
                                    names: str = "%d",
                                    vp_type: Union[str, dict] = "Unknown",
                                    diameter=35.0,
                                    rmax=None, name='') -> XConfiguration:
    ...
    fc = XConfiguration(location=location, names=anames, mount=mounts, xyz=antxyz,
                        vp_type=find_vptype_from_name(anames, vp_type),
                        diameter=diameters, name=name)
    return fc


def create_xconfiguration_from_SKAfile(antfile: str,
                                       mount: str = 'azel',
                                       names: str = "%d",
                                       vp_type: Union[str, dict] = "Unknown",
                                       rmax=None, name='', location=None) -> XConfiguration:
    """ Define XConfiguration from a SKA format file
...
def create_xconfiguration_from_MIDfile(antfile: str, location=None,
                                       mount: str = 'azel',
                                       vp_type: Union[str, dict] = "Unknown",
                                       rmax=None, name='') -> XConfiguration:
    """ Define XConfiguration from a SKA MID format file
    ...
    antxyz = numpy.genfromtxt(antfile, skip_header=5, usecols=[0, 1, 2], delimiter=" ").astype('float')
...
def limit_rmax(antxyz, diameters, names, mounts, rmax):
    ...
    if rmax is not None:
        ...
        log.debug('create_xconfiguration_from_file: Maximum radius %.1f m includes %d antennas/stations' %
                  (rmax, antxyz.shape[0]))
        diameters = diameters[r < rmax]
        names = numpy.array(names)[r < rmax]
        mounts = numpy.array(mounts)[r < rmax]
    else:
        log.debug('create_xconfiguration_from_file: %d antennas/stations' % (antxyz.shape[0]))
    return antxyz, diameters, names, mounts


def create_LOFAR_xconfiguration(antfile: str, location,
                                rmax=1e6) -> XConfiguration:
    """ Define XConfiguration from the LOFAR XConfiguration file
...
def create_named_xconfiguration(name: str = 'LOWBD2', **kwargs) -> XConfiguration:
    """ Create standard XConfigurations e.g. LOWBD2, MIDBD2

    Possible XConfigurations are::
        LOWBD1
        LOWBD2
        LOWBD2-core
        LOW == LOWR3
    ...
    :param name: name of XConfiguration LOWBD2, LOWBD1, LOFAR, VLAA, ASKAP
    ...
    if name == 'LOWBD2':
        ...
        log.debug("create_named_xconfiguration: %s\n\t%s\n\t%s" % (name, location.geocentric, location.geodetic))
        fc = create_xconfiguration_from_file(antfile=rascil_data_path("configurations/LOWBD2.csv"),
        ...
        location = low_location
        log.debug("create_named_xconfiguration: %s\n\t%s\n\t%s" % (name, location.geocentric, location.geodetic))
        fc = create_xconfiguration_from_file(antfile=rascil_data_path("configurations/LOWBD1.csv"),
                                             vp_type="LOW",
        ...
        location = low_location
        log.debug("create_named_xconfiguration: %s\n\t%s\n\t%s" % (name, location.geocentric, location.geodetic))
        fc = create_xconfiguration_from_file(antfile=rascil_data_path("configurations/LOWBD2-CORE.csv"),
                                             vp_type="LOW",
                                             location=location, mount='xy', names='LOWBD2_%d',
                                             diameter=35.0, name=name, **kwargs)
        ...
        location = low_location
        log.debug("create_named_xconfiguration: %s\n\t%s\n\t%s" % (name, location.geocentric, location.geodetic))
        fc = create_xconfiguration_from_MIDfile(antfile=rascil_data_path("configurations/ska1low_local.cfg"),
                                                vp_type="LOW",
                                                mount='xy', name=name, location=location, **kwargs)
    elif (name == 'MID') or (name == "MIDR5"):
        location = mid_location
        log.debug("create_named_xconfiguration: %s\n\t%s\n\t%s" % (name, location.geocentric, location.geodetic))
        fc = create_xconfiguration_from_MIDfile(antfile=rascil_data_path("configurations/ska1mid.cfg"),
                                                vp_type={"M0": "MEERKAT", "SKA": "MID"},
                                                mount='azel', name=name, location=location, **kwargs)
    elif name == 'MEERKAT+':
        location = meerkat_location
        log.debug("create_named_xconfiguration: %s\n\t%s\n\t%s" % (name, location.geocentric, location.geodetic))
        fc = create_xconfiguration_from_MIDfile(antfile=rascil_data_path("configurations/mkatplus.cfg"),
                                                vp_type={"m0": "MEERKAT", "s0": "MID"},
                                                mount='azel', name=name, location=location, **kwargs)
    elif name == 'ASKAP':
        location = EarthLocation(lon=+116.6356824 * u.deg, lat=-26.7013006 * u.deg, height=377.0)
        log.debug("create_named_xconfiguration: %s\n\t%s\n\t%s" % (name, location.geocentric, location.geodetic))
        fc = create_xconfiguration_from_file(antfile=rascil_data_path("configurations/A27CR3P6B.in.csv"),
                                             vp_type="ASKAP",
        ...
        fc = create_LOFAR_xconfiguration(antfile=rascil_data_path("configurations/LOFAR.csv"), location=location)
    elif name == 'VLAA':
        location = EarthLocation(lon=-107.6184 * u.deg, lat=34.0784 * u.deg, height=2124.0)
        log.debug("create_named_xconfiguration: %s\n\t%s\n\t%s" % (name, location.geocentric, location.geodetic))
        fc = create_xconfiguration_from_file(antfile=rascil_data_path("configurations/VLA_A_hor_xyz.csv"),
                                             location=location,
                                             mount='azel',
                                             names='VLA_%d',
                                             vp_type="VLA",
        ...
        raise ValueError("No such XConfiguration %s" % name)
    return fc
